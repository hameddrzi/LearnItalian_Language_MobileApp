...
const ExamFirstUniversityScreen = ({ navigation }) => {
  ...
  // جایگزینِ تابع handleFinishExam کن
const handleFinishExam = () => {
  // محاسبه تعداد پاسخ‌های درست
  let correctAnswers = 0;
  examQuestions.forEach((q, idx) => {
    if (selectedAnswers[idx] === q.correctAnswer) correctAnswers++;
  });

  // نمره و زمان مصرف‌شده
  const score = Math.round((correctAnswers / examQuestions.length) * 100);
  const timeSpent = 45 * 60 - timeRemaining; // ثانیه

  // داده‌هایی که به اسکرین‌های نتیجه می‌فرستیم
  const payload = {
    score,
    correctAnswers,
    totalQuestions: examQuestions.length,
    selectedAnswers,          // {0:3, 1:2, ...}
    questions: examQuestions, // برای نمایش سؤال و جواب درست/نادرست
    timeSpent,                // برای نمایش زمان
  };

  // رفتن به صفحه موفق/ناموفق
  if (score >= 60) {
    navigation.replace('SuccesExamFirst', payload);
  } else {
    navigation.replace('FaildExamFirst', payload);
  }
};
...
export default ExamFirstUniversityScreen;
