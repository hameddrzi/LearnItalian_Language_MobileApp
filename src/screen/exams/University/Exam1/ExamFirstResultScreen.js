...
// ⬇️ اضافه شد: ایمپورتِ متن‌های جدا
...
const ExamFirstResultScreen = ({ navigation, route }) => {
  const {
    ...
    timeSpent = 0,
    // ⬇️ اگر از بیرون پاس دادی (route.params.notes) اولویت با اونه
  ...
  const assistantData = useMemo(() => {
    ...
    const title = (note && note.title) ? note.title : 'Assistant';
    const text = (note && note.text) ? note.text : 'برای این سؤال هنوز توضیحی ثبت نشده است.';

    return { q, pair, title, text, index: assistantIndex };
        ...
        <View style={styles.modalCard}>
          ...
          {assistantData && (
            ...
            >
              ...
              <Text style={styles.aiText}>{assistantData.text}</Text>
            </ScrollView>
          )}
...
// ... styles همان نسخهٔ قبلی شماست (بدون تغییر)
